    ...
    const pullEditions = async () => {
        ...
        for (let i = 0; i < tempCache.metadata.length; i++) {
            ...
            if (tempCache.metadata[i].info.editionNonce != null) {
                ...
                edition = await (0, actions_1.getEdition)(tempCache.metadata[i].info.mint);
            }
            setOf100MetadataEditionKeys.push(edition);
            if (setOf100MetadataEditionKeys.length >= 100) {
                editionPromises.push((0, getMultipleAccounts_1.getMultipleAccounts)(connection, setOf100MetadataEditionKeys, 'recent'));
...
const metadataByMintUpdater = async (metadata, state, all) => {
    ...
    if (all ||
        (0, isMetadataPartOfStore_1.isMetadataPartOfStore)(metadata, state.store, state.whitelistedCreatorsByCreator)) {
